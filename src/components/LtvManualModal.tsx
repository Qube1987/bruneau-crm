...
import { X, Save, Plus, Search, User, TrendingUp } from 'lucide-react';
import { supabaseApi, Prospect } from '../services/supabaseApi';
import { extrabatApi } from '../services/extrabatApi';

interface LtvManualModalProps {
  onClose: () => void;
  onLtvCreated: () => void;
}

const LtvManualModal: React.FC<LtvManualModalProps> = ({ onClose, onLtvCreated }) => {
  ...
  const handleCreateLtvEntry = async () => {
    ...
    try {
      const opportuniteData = {
        ...
        description: ltvForm.description,
        statut: 'devis-gagne' as const,
        suivi_par: 'Quentin BRUNEAU',
        statut_final: 'gagne' as const,
        montant_estime: ltvForm.montant ? parseFloat(ltvForm.montant) : undefined,
      };

      const opportunite = await supabaseApi.createOpportunite(opportuniteData, false);

      const dateInstallation = ltvForm.date_installation || new Date().toISOString();
      ...
      const chantierData = {
        ...
        chantier_realise: true,
        date_commande_passee: dateInstallation,
        date_commande_recue: dateInstallation,
        date_chantier_planifie: dateInstallation,
      ...
      alert('✅ Entrée LTV créée avec succès ! La checklist d\'actions a été générée automatiquement.');
      onLtvCreated();
    } catch (error) {
      console.error('Erreur lors de la création de l\'entrée LTV:', error);
      alert('Erreur lors de la création de l\'entrée LTV');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-6 border-b border-gray-200 bg-green-50">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold text-gray-900">Ajouter un client en optimisation LTV</h3>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="space-y-4">
              ...
              {extrabatResults.length > 0 && (
                <div className="border border-green-200 rounded-lg p-4 bg-green-50">
                  <h4 className="font-semibold text-green-900 mb-2">Résultats Extrabat</h4>
          ...
          {step === 'details' && selectedProspect && (
            <div className="space-y-4">
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <h4 className="font-semibold text-green-900 mb-1">Client sélectionné</h4>
                <p className="text-green-800">
                  {selectedProspect.nom} {selectedProspect.prenom || ''}
                </p>
                {selectedProspect.telephone && (
                  <p className="text-sm text-green-700">{selectedProspect.telephone}</p>
                )}
              </div>

              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                <p className="text-sm text-blue-800">
                  Cette entrée sera créée directement dans "Optimisation de la valeur à vie" avec une checklist d'actions commerciales générée automatiquement.
                </p>
              </div>
              ...
              <div>
                ...
                <textarea
                  value={ltvForm.description}
                  onChange={(e) => setLtvForm({ ...ltvForm, description: e.target.value })}
                  rows={3}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
                  placeholder="Type de projet, équipements installés..."
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="date"
                    value={ltvForm.date_installation}
                    onChange={(e) => setLtvForm({ ...ltvForm, date_installation: e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    value={ltvForm.montant}
                    onChange={(e) => setLtvForm({ ...ltvForm, montant: e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500"
                    placeholder="€"
                  />
                </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                >
                  ...
                  {isLoading ? 'Création...' : 'Créer l\'entrée LTV'}
                </button>
              </div>
            </div>
          )}
